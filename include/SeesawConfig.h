...
#define CONFIG_GPIO_MASK (((unsigned long long) 0xFFFFFFFFFFFFFFFF) ^ ( ((uint64_t)CONFIG_USB << CONFIG_USBPIN_USB_DM) | ((uint64_t)CONFIG_USB << CONFIG_USBPIN_USB_DP) \
		...
		| ((uint64_t)CONFIG_ENCODER  << CONFIG_ENCODER0_A_PIN) \
		| ((uint64_t)CONFIG_ENCODER  << CONFIG_ENCODER0_B_PIN) \
...
#define CONFIG_NUM_AO 	(1 + CONFIG_ADC + CONFIG_DAC + CONFIG_TIMER + \
						...
						CONFIG_SPI_SLAVE + CONFIG_DAP + CONFIG_NEOPIXEL + CONFIG_TOUCH + \
                        CONFIG_KEYPAD + CONFIG_ENCODER + CONFIG_BLINK)
